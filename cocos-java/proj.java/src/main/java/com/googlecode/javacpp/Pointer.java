...
//        }
     ...
     */
    public void init(long allocatedAddress, int allocatedCapacity, long deallocatorAddress)
    {
        address = allocatedAddress;
        position = 0;
        limit = allocatedCapacity;
        capacity = allocatedCapacity;
        deallocator = new NativeDeallocator(this, deallocatorAddress,getClass().getName());
//        deallocator(new NativeDeallocator(this, deallocatorAddress));
        ...
        implements Deallocator
    {
        ...
        NativeDeallocator(Pointer p, long deallocatorAddress,String className)
        {
            super(p, null);
            this.deallocator = this;
...
//                System.out.println("deallocating 0x" + Long.toHexString(allocatedAddress) + " 0x" + Long.toHexString(deallocatorAddress)+" "+className);
     ...
     */
    public void deallocate(@Cast("bool") boolean deallocate)
    {
    ...
    {
        Class c = getClass();
        if (c == Pointer.class || c == Pointer.class) {
            // default to 1 byte
            return 1;
        }
        else {
